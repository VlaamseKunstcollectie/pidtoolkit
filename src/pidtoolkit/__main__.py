...
def main(args):
    ...
    export_directory = args.dest if args.dest else os.path.dirname(os.path.abspath(import_file))
...
if __name__ == "__main__":
    ...
    parser.add_argument('csv', required=True, help="Path to CSV file with object numbers")
    parser.add_argument('-c', '--config', required=True,
                        help="Path to generation configuration file. Required.")
    parser.add_argument('-d', '--dest',
                        help="Path to destination directory")
    parser.add_argument('-u', '--urls', default="false", choices=['true', 'false'],
                        help="Generate CSV with URLs for CultURIze. Default 'false'.")
